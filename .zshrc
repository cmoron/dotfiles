...
for _fzf in /opt/homebrew/opt/fzf/shell /usr/share/fzf /usr/share/doc/fzf/examples; do
...
# iTerm2 shell integration zsh (si iTerm2 utilisé)
...
alias df='df -h'
alias du='du -h'
alias free='vm_stat'  # macOS n'a pas free
...
# Notif macOS si commande > 30s
_notif-preexec() { _cmd_start=$EPOCHSECONDS; _cmd_name="$1" }
_notif-precmd()  {
    local last_status=$?
    [[ -z "$_cmd_start" ]] && return
    local elapsed=$(( EPOCHSECONDS - _cmd_start ))
    if (( elapsed > 30 )) && command -v osascript >/dev/null; then
        local status_text="done"; (( last_status != 0 )) && status_text="failed"
        local safe_cmd="${_cmd_name//\"/}"
        osascript -e "display notification \"${safe_cmd} (${elapsed}s, ${status_text})\" with title \"shell\"" &>/dev/null
    fi
